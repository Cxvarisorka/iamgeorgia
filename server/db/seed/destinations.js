/**
 * The destination catalogue: eight Georgian destinations with their editorial
 * copy, galleries, attractions and travel notes.
 *
 * This prose is the platform's real content. It lived in the client as a
 * prototype fixture (`client/data/destinations.ts`) until the site started
 * reading destinations from the API; the data moved here with the seed that
 * loads it, the same way the tours and services did. Shape mirrors the
 * client's `Destination` type. Loaded by `scripts/seed-catalogue.js`, which
 * adds the tree (under a `georgia` root), coordinates and types on top.
 */
